...
export class PB2 {
  ...
  // ======================================================
  // DO RESET ROUTE (correct placement)
  // ======================================================
  async handleReset() {
  ...
  async fetch(request) {
    ...
    if (path === "/assign" && request.method === "POST") return this.handleAssign(request);
    if (path === "/removeCaptain" && request.method === "POST") return this.handleRemoveCaptain(url);
    if (path === "/bump" && request.method === "POST") return this.handleBump();
    if (path === "/stream" && request.method === "GET") return this.handleStream();

    if (path === "/reset" && request.method === "POST") return this.handleReset();
...
export default {
  async fetch(request, env) {
    ...
    // RESET INDEX
    // ======================================================
    if (pathname === "/api/reset-index") {
      await env.PB.put("PB_INDEX", JSON.stringify([]));
    ...
    if (pathname.startsWith("/api/pb/")) {
      ...
      // ======================================================
      // PB RESET (correct placement)
      // ======================================================
      if (parts[3] === "reset" && request.method === "POST") {
        const doRes = await stub.fetch("https://do/reset", { method: "POST" });
        const doData = await doRes.json();
        return json(doData);
      }
